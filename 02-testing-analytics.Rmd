...
####How do we test?
In order to get some sense of the quality of an application, developers can apply different
methods. For the quality of code or the quality of testing different metrics can be used, of which
on of the most popular metric is the code coverage metric. Alternatives are, for example,
acceptance tests, the number of defects in the last week, or defects per Line of Code (LOC)
[@GAROUSI20131354]. Code coverage might not be the best indicator for the extensiveness of testing,
as according to @Beller:2015:DT:2819009.2819101 a code coverage 75% can possibly be reached with
only spending less than a tenth of the total development time on testing. This might also have to
do something with treating the metric, where developers try to uplift the value of code coverage by
hitting many lines with only a few test cases. It could also be the case that tests are too simple.
In @marsavina2014 it was observed that test cases were rarely updated when changes related to
attributes or methods in the production code were made. Possible explanations for this are that
these changes were not significant or the tests were too simple and were likely to pass. This also
fits with the findings of @ROMANO201764, where they claim that "[d]evelopers write quick-and-dirty
production code to pass the tests, do not update their tests often, and ignore refactoring."
Besides older tests rarely being updated for changed code, it was also observed by
@pinto2012understanding that a significant number of new tests that are added, were not necessarily
added to cover new code but rather to exercise the changed parts of the code after the program is
modified. This finding fits with the observation of @marsavina2014, who found whenever numerous
condition related changes are conducted in the production code base, test cases are created or
deleted in order to address the modified branches. Older production code lines, therefore, may stay
untouched by any test cases. Lines uncovered by any code coverage tool should be indicated and
signaled to the developer, whereas the most commonly used coverage metrics are branch coverage and
conditional coverage [@GAROUSI20131354]. As both branch coverage and conditional coverage require
multiple different conditions for if-statements, it may possibly be that the absolute number of
missed lines of production code by tests is very low.

####Co-evolution
In a case study conducted by @zaidman2011studying, there was no evidence found for an increased
activity of testing before a release. However, they were capable of detecting periods of increased
...
@beller2015, where programmers who claim they practice TDD neither follow it strictly nor do they
...
@beller2015; only 1.7% of the developers followed seem to follow the strict TDD definition, where
even most of these developers only practice this strict definition in less than 20% of their time.
...
Although @pinto2012understanding found the investigation in automated test repairing not a 
promising research avenue, as these techniques would require manual guidance which could end up 
being similar to traditional refactoring tools, research is still being performed in this field. 
...
While full automated test suite generation is not there yet, @bowring2014obsidian introduced a 
tool that generates the templates for tests, which guarantees compilation, supports exception 
handling and finds suitable location for the test. Developers still need to fix the oracle tests 
...
techniques for enhancing an existing unit test generation system. In experiments using an 
industrial system, the generated tests achieved good coverage and mutation kill score, were 
readable by the product’s developers and required few edits as the system under test evolved. 
@dulz2013model found that by directed adjusting specific probability values in the usage profile 
of a Markov chain usage model it is relatively easy to generate abstract test suites for different 
user classes and test purposes in an automated approach. By using proper tools, like the TestUS 
Testplayer even less experienced test engineers will be able to efficiently generate abstract test 
cases and to graphically assess quality characteristics of different test suites.
...
identifying failing test cases compared to the traditional risk measure. Using this method in 
...
better understand test-suite evolution (@pinto2013). For the time being the practical implications of
...
(@supportingtestsuite) argues that evolving test cases is an expensive and time-consuming activity,
for which automated approaches reduce the pressure on developers. Shamshiri et al. 
(@shamshiri2018automatically) argues that automated generation of unit tests does not end up
generating realistic tests and that the effectivity of developers writing manual tests is equal
to developers using automatically generated tests. They, therefore, call for the use of more
realistic tests. This suggests that automated test generation is still a topic of interest, which
will consequently likely be researched in order to find a way to generate realistic tests. 

####Risk-driven testing####
Risk-driven testing has been receiving attention recently. Researchers have been looking for
...
the same project and cross-project prediction. Given the currently researched techniques regard
history based implementations, it's like that these techniques will be subject to further research
later on.  
