...
## Research protocol
For this paper the Kitchemham survey method was applied. For this method a protocol has to be
...
* Reverse engineering and reengineering for software evolution systems.
* Proposes a system (called Kenyon) for the configuration and (meta)data storage of actual code evolution.
* Many systems normally use CVS as storage, which limits the applicability for industrial systems where CVS is often not used. Although CVS is commonly considered acceptable.
* Created to facilitate both the creation of new software evolution analysis tools as well as the sharing of data between such tools.
* Kenyon included configuration retrieval, fact extractor invocation, and database storage and access.
...
* Code decay: introducing new features, accommodate new hardware and repair faults.
* Code decay becomes increasingly harder over time.
* Code decay does not come from the code itself but from its environment.
* Research based on a single, large, fifteen-year old real-time software system for telephone switches.
* A unit of code is decayed if it is harder to change that it should be, measured in terms of effort, interval, and quality.
* Earlier was reported that it is nearly-impossible to add new code to aged systems without introducing faults.
* In the system, evidence for code decay was found. While, at the same time, evidence of dramatic, widespread decay is lacking. According to the paper also because the system is only fifteen years old, successful, and new features can still be added.
* The authors anticipate that all projects of sufficiently large scale will, to some extent, will exhibit decay. In other words, code decay is a generic phenomenon.
* Current investigations focus on preventing code decay. Whether code decay can ultimately be fatal is unclear. 
...
* Risk was reduced towards the end - but weak model to validate this.
* Reliability increased towards the end - also; weak model.
* Insight into complex interrelationship of customer and producer risk and reliability.
* // weak paper: old and little information //
