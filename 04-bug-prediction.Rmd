...
However, as noted in reference @DAmbros2012, reliably predicting which parts of source code are the most 
...
1. Keyword search using search engines (Google Scholar, Scopus, ACM Digital Library, IEEE Explorer).
...
and reference @Catal2009review found code metrics were the most commonly studied metrics in literature. 
...
Reference @DAmbros2012 articulates a rationale behind using code complexity for bug prediction --- 
...
References @Radjenovic2013, @Malhotra2015 and @Catal2009review found the metrics suite given by Chidamber 
and Kemerer (CK, see table X) to be the most common among the OO metrics and it was used in 
reference @Gyimothy2005 to study the predictive power of object-oriented metrics.

A drawback of static code metrics is identified in reference @Rahman2013, 
...
Reference @DAmbros2012 notes one of the rationales for using these metrics in a predictive model,
faults are introduced by changes in software and should be studied. 

Some examples of process metrics as used in references @Moser2008 and @Rahman2013 include code churn, 
...
predictors based on code changes in reference @rahman2011.

Reference @Radjenovic2013 groups metrics that are derived by looking at the source code change history as 
...
Reference @Moser2008 compared the power of code and process metrics by analysing the Eclipse project 
and found the process metrics outperform code metrics. 
Reference @Rahman2013 observed that process metrics perform better as predictors compared to code metrics 
and are more stable across releases. 
However, reference @DAmbros2012 note that such results should be taken with a grain of salt; 
...
Reference @Giger2011 explores potential advantages of using more fine-grained source code changes, capturing 
...
Reference @Matsumoto2010 shows that modules touched by more developers are more likely to contain more bugs.
They also show that using developer based metrics in combination with other metrics can improve
prediction results. Reference @rahman2011 has shown that developer experience 
has no clear correlation with how much faults they introduce.

Reference @Lee2011 tries to predict faults by tracking micro-interactions of developers.
...
Reference @DiNucci2018 shows scattering metrics, which uses data about how focussed a developers work is.
...
However, reference @DAmbros2010 contradicts this statement.
...
Reference @wang2016 uses ASTs in combination with a machine learning approach for bug prediction.
...
Reference @Jiang2008 focuses solely on how to evaluate the algorithm’s performance, 
...
Finally, we came across reference @DAmbros2010, in which the authors choose to use 
...
this is because, citing reference @Lessman2008: “The AUC was recommended as the primary accuracy indicator 
...
It is also worth noting the conclusion of reference @Shepperd2014, 
...
Where reference @Lewis2013 even mentions that their tool had no significant effect on developers.
Reference @kim2007 says software managers could use a list of bug-prone files to allocate more
...
The only paper that tackles this question is reference @Lewis2013, 
...
and we, as in reference @Lewis2013, think that maybe bug prediction is supposed to be used for software 
