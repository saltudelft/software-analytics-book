...
field of software development for mobile devices, considering energy consumption determines the battery-life and limites the usage time of the device. Additionally, battery life is seen as very
...
## Methodology
In order to answers the research questions, over 30 papers in the selected field have been
selected. These papers have been found by searching on Google Scholar with a set of filters. First
off, only papers from the following journals and conference are selected:
...
The following criteria is given by the activity and how relatively new the field is. The
preference is given to papers which are published not too long ago, preferably after 2012. But if
well cited, and with interesting findings, older papers can also be used.
...
the potential program paths along which energy bugs that may occur.
...
noted that the paper does not test this on actual Android applications but rather the Rosetta Code 
repository.
...
measuring of energy-consumption is not actually as straight-forward as one may think. In a number
of papers it is explained how hardware-based solutions are accurate, but require expensive
components. This is especially in the field of mobile devices a serious issue. On the other hand, software-based solutions are cheaper but less accurate. Now there is work being done on creating
more accurate software solutions [@@NPPPZL2017]. Early work shows promising results in combining
the best of both worlds in order ot obtain improved results.
