...
published at top conferences. These five were selected because the papers discuss the software side
of performance and runtime analytics which is more consistent with the scope of this book.
...
on the battery duration and hence limiting the usage time of the device. Additionally, battery life 
...
field either [@PHA2016]. In an analysis on questions posted on StackOverflow on the topic of 
...
The following criteria is given by the activity and relative newness of the field. The preference 
is
...
overview of all state of the art tools and guidelines that satisfy our search criteria. This 
overview is used to answer **RQ1**.
...
### Guidelines
Another aspect of the state of the art are the currently proposed guidelines for energy awerness 
and improvements. To answer **RQ1**, seven papers have been selected who present guidelines. The 
research by Cruz et. al. [@CA2017] shows that, interesting enough, the best practices provided by 
...
efficiency.
From the research by McIntosh et. al.[@MSA2018] it is noted that machine learning is now also 
implemented by excited developers into their apps, having an impact on the mobile device's battery 
life. They have combined empirical measurements of different machine learning algorithm 
...
are valid. The topics they investigated included optimizing the sending of HTTP packets, memory 
usage, array lengths, static invocations, and field access. Their work gives guidance for mobile 
app developers. Especially bundling network packets up to a certain size and using certain coding 
practices for reading array length information, accessing class fields, and performing invocations 
all led to reduced energy consumption.
...
comparing it to other languages as can be seen Table [ref:to table nr].  However, it should be 
noted
...
for
aiding developers in making energy-aware decisions. One example of such a tool that attempts to 
assist the developer in making energy friendly descisions is JStanley [@PSCS2018]. However, both 
the paper itslef and our own replication of that study indicate that this work is not anywhere 
close to ideal. It only concerns static analysis of the code and is very limited in usibility. 
This prevents such tools of being used in practice. Additionally, there is the problem of how to 
actually measure the energy being consumed. A number of papers explains how this is not actually 
an easy task. Moura et al. also alude to the fact that developers that do use third-party 
energy-management tools often lack trust in the accuracy of the tools that they are using.

### Answering SQ3
As discussed in the previous paragraph, the measuring of energy-consumption is not actually as 
straight-forwad as one may think. In a number of papers it is explained how hardware-based 
solutions are accurate, but require expensive components. This is especially in the field of 
mobile devices a serious issue. ON the other hand, software-based solutions are cheaper but less 
accurate. Now there is work being done on creating more accurate software solutions [@@NPPPZL2017].
...
language used for implementation has an influence on the energy 
...
beingtested with benchmarks. However, it is always the question whether these benchmarks are 
...
verify that the enegy-saving commits actually have an impact on the overall performance of the 
software. That is both in terms of energy-consumption as well as metrics for the performance 
related to usebility. Similarly, an other direction is verifying that the enery-saving techniques 
are consistent acrosss platforms.
...
energy awareness in regards to developing applications for mobile devices, and more specific 
Android, needs more attention. A lot of research is done in the field, resulting guidelines and 
tools to help developers. But these guidelines are very generic, and also conclude that energy 
awarness is project specific. The tools presented by papers hold claims to improvements, but for 
example the tested tool JStanley is not as easy to use as claimed by the authors. More research 
into the claims made by the authors of these tools is needed.
In practice it is observed that the main language for creating Android apps (Java) isn't the most 
