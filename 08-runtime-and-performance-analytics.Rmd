...
In this chapter, we discuss the field of performance and runtime analytics. This chapter however does not
...
### Guidelines
Another aspect of the state of the art are the currently proposed guidelines for energy awerness and improvements. To answer **RQ1**, seven papers have been selected who present guidelines. The research by Cruz et. al. [@CA2017] shows that, interesting enough, the best practices provided by Google fall short in addressing energy consumption. However they come up with guidelines that do give improvements. Their guideline shows that correct usage of Android methods such as _iewHolder_, _DrawAllocation_, _WakeLock_, _ObsoleteLayoutParam_, and _Recycle_ improved energy efficiency.
From the research by McIntosh et. al.[@MSA2018] it is noted that machine learning is now also implemented by excited developers into their apps, having an impact on the mobile device's battery life. They have combined empirical measurements of different machine learning algorithm implementations with complexity theory to provide the guideline that some implementations of algorithms, such as J48, MLP and SMO, generally perform better than others. However they also recommend that for optimal results developers must consider their own specific application since many factors, e.g. dataset size, can influence the performance.
Finally Li et. al.[@LH2014] researched commonly recommended energy saving practices, to see if they are valid. The topics they investigated included optimizing the sending of HTTP packets, memory usage, array lengths, static invocations, and field access. Their work gives guidance for mobile app developers. Especially bundling network packets up to a certain size and using certain coding practices for reading array length information, accessing class fields, and performing invocations all led to reduced energy consumption.
...
* **SQ3:** How certain are researchers and developers about the validity of research and tools?
...
aiding developers in making energy-aware decisions. One example of such a tool that attempts to assist the developer in making energy friendly descisions is JStanley [@PSCS2018]. However, both the paper itslef and our own replication of that study indicate that this work is not anywhere close to ideal. It only concerns static analysis of the code and is very limited in usibility. This prevents such tools of being used in practice. Additionally, there is the problem of how to actually measure the energy being consumed. A number of papers explains how this is not actually an easy task. Moura et al. also alude to the fact that developers that do use third-party energy-management tools often lack trust in the accuracy of the tools that they are using.

### Answering SQ3
As discussed in the previous paragraph, the measuring of energy-consumption is not actually as straight-forwad as one may think. In a number of papers it is explained how hardware-based solutions are accurate, but require expensive components. This is especially in the field of mobile devices a serious issue. ON the other hand, software-based solutions are cheaper but less accurate. Now there is work being done on creating more accurate software solutions [@@NPPPZL2017].
...
One of the fields of research we looked at in the previous sections was the relation between programming languages and energy consumption. Multiple papers suggest that the programming language used for implementation has an influence on the energy 
...
Furthermore, the outcomes of the papers introducing these tools are not comparable. Testing all these tools on the same project would give better insights in what tools are actually useful for the developers to use in terms of gaining as much energy efficiency as possible. 

Even though, to our knowledge, there have not been any studies into whether the tools described in the research are actually been used in practice, there have been some studies into energy-saving practices on Github. For instance, studies into the commit messages [@BLXWT2016; @MPEC2015], have shed some light into the state of practice. These studies looked into the commit messages containing keywords related to energy consumption and classified the corresponding code. The future research directions that are indicated by such papers is that it will be important to verify that the enegy-saving commits actually have an impact on the overall performance of the software. That is both in terms of energy-consumption as well as metrics for the performance related to usebility. Similarly, an other direction is verifying that the enery-saving techniques are consistent acrosss platforms.
...
## Conclusion
Having answered the research questions of this chapter in the sections above, we can conclude that energy awareness in regards to developing applications for mobile devices, and more specific Android, needs more attention. A lot of research is done in the field, resulting guidelines and tools to help developers. But these guidelines are very generic, and also conclude that energy awarness is project specific. The tools presented by papers hold claims to improvements, but for example the tested tool JStanley is not as easy to use as claimed by the authors. More research into the claims made by the authors of these tools is needed.
In practice it is observed that the main language for creating Android apps (Java) isn't the most energy efficient option. Furthermore developers often do not give enough priority to energy saving options, and when they do they are often unsure about the effects of their changes. 
