...
systematic reviews by @kitchenham2004procedures.
...
The interested can find our research protocol in detail on
[this wiki](https://github.com/nsalminen/software-analytics-book/wiki).

<!-- NOTE: everything from here until the "Answers" section
     will be moved to the wiki page -->

In this appendix, we will describe in detail how we applied the protocol for
performing systematic literature reviews by @kitchenham2004procedures.
...
|-----------|----------|-------|
| 1 | 16/9/18 | - Develop the search strategy<br> - Collect initial publications |
| 2 | 23/9/18 | Write full research protocol |
| 3 | 30/9/18 | - Collect additional literature according to the protocol<br> - Perform data extraction |
| 4 | 7/10/18 | - Perform data synthesis<br> - Write final version of the chapter |
...
@khomh2015a have looked into the effects of switching from traditional to rapid release cycles 
...
earlier in the software execution. @mantyla2015a have also considered data from Mozilla 
Firefox, and has examined the impact of release engeering on testing efforts. Observations of 
the paper conclude that the rapid release cycle perform more test executions per day, but 
...
rather than causation. @da2014a has further zoomed into the integration of addressed issues, 
...
excluded from releases. Similar conclusions based on Mozilla Firefox were made in 2016, when 
@da2016a found that minor-traditional releases tend to have less integration delay than 
major/minor-rapid releases.

@castelluccio2017a has examined the practice of *patch uplifting* in the release management at 
...
@teixeira2017a has described OpenStack's shift to a liberal six-month release cycle. As this 
...
time-based releases. OpenStack encourages regular releases but also attemps to include new 
features at each regular release.

Rather than focussing on topics such as issue and delays, @poo-caamano2016a focusses on the 
communication in release engineering in the cases of GNOME and OpenStack. Through analyzing 
over 2.5 years of communication, the paper has made a number of observations. The paper found 
that developers tend to communicate through blogs, bug trackers, conferences and hackfests, 
and that a release team is set to define requirements, quality standards and coordination 
through (direct) communcation. Although only the mailing lists of the projects were studied, 
defined challenges include keeping everyone informed and engaged, monitoring changes and 
setting priorities in cross-project coordination.

@laukkanen2018a has described what effects modern release engineering have on software with 
different organizational contexts. This study specifically focusses on continuous deployment 
(CD) practices. The paper has found that high internal quality standards combined with the 
...
environments. More about the factors that can play a role are addressed by @rodriguez2017a, 
...
case study by @laukkanen2018a it was found that a complex automated build and
...
higher quality, how anti-patterns in the build design can be refactored,
...
- **RQ2:** _What aspects of modern release engineering have been studied
  in software analytics research so far?_
    At this point in time, case studies have mainly focussed on the resulting factors of switching 
    from a traditional release cycle to a rapid release cycle, and what effects this has in various
    organizational contexts. As all included studies suggest, more emperical studies are needed to be
