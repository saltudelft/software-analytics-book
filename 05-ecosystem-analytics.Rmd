...
libraries to use the functionality (for example parsing JSON) of these libraries, without having
to develop this functionality itself. Moreover, multiple languages, such as Python and Rust,
provide package managers (pip[^1] and Cargo[^2] respectively) which can be used to easily manage
this third-party functionality, as well as distribute it.
...
ecosystem_. As stated by @Messerschmitt2003, a _software ecosystem_ is "a collection of software
products that have some given degree of symbiotic relationships." Another, similar definition is
given by @Lungu2009: "A software ecosystem is a collection of software projects which are
developed and co-evolve in the same environment." @Mens2013 extends this definition, "by
explicitly considering the communities involved (e.g. user and developer communities) as being
part of the software ecosystem." @Stallman2002 opposes the overall notion of calling this
...
consultations with experts in the field, among other things. Our search strategy can be split in
...
There are more ecosystems which have the same properties, such as RubyGems (Ruby) and Crates.io
(Rust)[@Kikas2017].
...
CRAN (R), npm (JavaScript), NuGet (.NET), Packagist (PHP) and RubyGems (Ruby). @Decan2018
therefore studies the most different ecosystems in one paper, relative to the papers studied in
this survey.
...
errors, thereby breaking the system that depends on it.

Developers tend to react poorly to changes in their dependencies; @Kula2017 has found that,
of the 4600 surveyed projects, 81.5% of the projects contain outdated dependencies which
can lead to security risks. Not only do developers not update their dependencies, according to
an empirically study done by @McDonnell2013 on the android API, they also do not update their
codebase with respect to the changes introduced by dependencies.

Regarding ecosystem health, @Constantinou2017 has researched which factors indicate that a
developer is likely to abandon an ecosystem. Their study, which analyzed GitHub[^3] issues and
...
activities and 3) for an extended period of time do not react or commit anymore. Another
interesting characteristic about ecosystem health, studied by @Kula2017-2, is the way in
which projects age over time. Their study found that the usage over time of 81.7% of 4,659
...
with both Python 2 and Python 3 by only using a subset of the Python 3 language. @Malloy2018
states that they are severely limiting themselves in their language capabilities, by not using
...
Based on the results of @Trockman2018, badges can have a positive impact on the speed at
...
results can be generalized beyond the main package managers for specific languages. This is
...
This is shown as well in the different results obtained by @Bogart2016, which shows that values
differ per ecosystem. This overall shows that there is a lot of space for future research to be
done in generalizing research beyond the already researched ecosystems.
...
_Table 4.1: Papers provided by MSc. Joseph Hejderup. The first column describes whether the paper of
...
| @Blincoe2015 | Ecosystems in GitHub and a method for ecosystem identification using reference coupling.                 | 2015 | Referene Coupling, Ecosystems, Technical Dependencies, GitHub, cross-reference | @Constantinou2017                                    |
| @Cox2015                               | Measuring Dependency Freshness in Software Systems                                                       | 2015 | software metrics, software maintenance | @Kikas2017                                     |
...
| @Malloy2017                           | Quantifying the transition from Python 2 to 3: an empirical study of Python applications.                 | 2017 | Python, programming language evolution, language features  | @Malloy2018                                         |
