...
managers are central repositories from which the packages, aswell as their dependencies, can
easily be retrieved.
...
explains that it is benificial to use this ecosystem, because the majority of their packages are
hosted on GitHub and Developers specify required packages in their project's dependency files.
Ofcourse, there are more ecosystems which have the same properties, such as RubyGems (Ruby) and
...
researches package incompatiblities and therefore uses a ten year period of the Debian i386
...
| --------- | ----------------- | ------------------ | ------- | ---------- | ------------ | ---------- |
| @Abdalkareem2017 | Use of trivial packages | Quantitative, Qualitative | - | npm, GitHub | npm | Used because it is assumed to be well implemented and tested (only 45% actually has tests) and increases productivity. Quantitative research has shown that 10% of NodeJS uses trivial packages, where 16.8% are trivial packages in npm |
| @Bavota2014 | Upgrading of dependencies | Quantitative, Qualitative | - | Apache | Apache | Upgrade done when bugfixes, but no API changes |
...
| @Cox2015 | Metric for dependency freshness of a system | Qualitative / Quantitative | - | Maven | Maven | Metric has been found and verified with Maven |
| @Decan2017 | Dependency Issues in OSS Packaging Ecosystems | Quantitative analysis | - | Eclipse, CRAM, npm | Eclipse, CRAM, npm | In all ecosystems, dependency updates result in issues, however the problems and solutions do vary. |
| @Kikas2017 | Package dependency networks | Quantitative | - | npm, RubyGems, Crates.io | npm, RubyGems, Crates.io | All ecosystems are growing and over time, ecosystems become less dependent on a single popular package. |
| @Robbes2012 | Developers response to API deprecation | Quantitative | - | Squeak, Pharo | Squeak, Pharo | API changes can have a large impact on ecosystem. Projects take a long time to adapt to an API change. |
...
changes cannot be solved in a uniform manner througout the ecosystem, but rather need a specific
...
provide "a metric to aid stakeholdersin deciding on whether the dependencies of a system should
