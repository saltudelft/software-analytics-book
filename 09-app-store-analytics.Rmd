...
Play store alone as of first quarter of 2018 [@appNumber]. These app stores, together with a
large amount of user-generated data associated with them present an unprecedented source of
information about the ecosystems of modern software platforms (such as Android and iOS) for mobile devices. Software developers and researchers use these valuable data to
gain new insights about how users use their apps, what they like, and what difficulties they
encounter. The same data can be also used by app store operators in the detection of malicious or misbehaving
apps, whose behavior does not match their app store description. Because these app stores are relatively new,
the research field of App Store Analytics is still not mature. However, because apps are used so
much nowadays, their investigation plays a vital role in the field of Software Engineering.
...
were published after 2015.  We then tried to answer the following research questions:
...
[@di2016would]** 
 ...
 - Remarks: the authors applied classification and summarization techniques on app reviews to
   reduce the effort required to analyze feedback from users. As it can be seen, the paper was focused
...
the field of Review Analytics. In their survey [@martin2015survey] Martin et al. proposed
...
and “Wearables”. In the following sections, we expand on each one of these categories with the revised
literature.
...
same problem was proposed in [@xie2016you]. In the paper, Xie et al., identified manipulated app
...
considering that their main purpose was to investigate the later. 
...
in order to generate a summary of the main bugs and features of apps. Taking into account the high
...
amount of apps (except for [@palomba2018crowdsourcing] that used 100), as stated in the previous
section, they use numerous reviews (more than 20,000). Additionally, as the aim of the works in
this section directly involve the developers, they completed the quantitatively apps-reviews-based
...
default. Scoccia et al. did a large-scale empirical study on this new system challenges by inspecting 4.3
...
negative user concerns were apps asking for too many permissions or asking at a bad timing for permissions.

**Responding to reviews**

Since 2013 developers can respond to reviews in the Google Play store. Also, Apple introduced the same
