...
work done by the authors and using to answer the research questions specifically for the
...
a database with only the relevant articles, the following inclusion criteria were applied to all
the hits. It is important to notice that in addition to the results obtained by searching with the
...
App Store Analysis” to categorize the existing literature. After analyzing posterior work,
...
Store and crowdsourcing site, to identify manipulated reviews.
...
store data. Considering that they manage to merge it from data from an external
...
significant number of reviews that were analyzed. This makes sense, considering that the
main purpose is to investigate the later. 

**Mapping user reviews to the source code**
...
- They include a preprocessing step, topic detection, topic interpretation and visualization of the
  results.
- A sample of 6 popular apps from (4) Google Play and (2) App Store. Employed app’s changelogs as the
  ground truth.
